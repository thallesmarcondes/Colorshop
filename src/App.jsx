...
import { printReceipt } from "./utils/printerUtils";
...
export default function ColorShopDashboard() {
  ...
  function imprimirVendaTermica(venda) {
    const nomeImpressora = "Printer POS-80";
    const linhas = [
      "==== INDUFARMA ====",
      `Nota de venda #${venda.id}`,
      `Cliente: ${venda.clienteNome}`,
      venda.vendedor ? `Vendedor: ${venda.vendedor}` : null,
      `Data: ${fmtDate(venda.data)}`,
      "--------------------------------",
      ...(venda.itens || []).map((l) => `${l.itemNome} x${l.qtd}  ${fmtUSD(l.subtotal)}`),
      "--------------------------------",
    ...
    printReceipt(nomeImpressora, linhas).catch((err) => {
      console.error(err);
      alert("Erro ao imprimir. Verifique se o QZ Tray está aberto no Mac.");
    });
  }
              ...
              <tr key={v.id} className="border-b border-gray-50">
                ...
                <td className="px-5 py-3 align-top">
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => imprimirVendaTermica(v)} className="text-gray-400 hover:text-emerald-700" title="Imprimir cupom (impressora térmica)">
                      <Receipt size={15} />
                    </button>
